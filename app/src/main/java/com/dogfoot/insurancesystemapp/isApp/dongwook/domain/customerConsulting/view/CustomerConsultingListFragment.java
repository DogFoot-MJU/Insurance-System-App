...
public class CustomerConsultingListFragment extends DogFootViewModelFragment {
    ...
    protected void initializeView() {
        View.OnClickListener listener = v -> {
            ...
            dataset.put(DogFootEntity.EDogFootData.ANSWERED_ID,items.get(selectedItem).getAnswer_id());
            this.valueInput=true;
            this.save();
            if(valueInput) {
                Navigation.findNavController(getView()).navigate(R.id.action_customerConsultingList_to_customerConsultingView);}
        };
        this.customerConsultingRecyclerAdapter = new CustomerConsultingRecyclerAdapter(this.getActivity(),listener);
